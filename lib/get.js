...
module.exports = opts => {
  ...
  // Config Proxy
  var proxyDefaults = undefined;
  if (opts.proxy) {
    var Proxy = require('../proxy').init(opts.proxy);

    if (opts.proxy.provider === 'tor' ||
        opts.proxy.provider === 'crawlera') {
      var proxyDefaults = Proxy;
    }
  }
    ...
    const Queue = async.queue((item, cb) => {
      ...
      async.setImmediate(() => {
        setTimeout(function() {
          ...
          // Proxy Request Things
          // Specifically for luminati.IO
          if (typeof Proxy !== 'undefined' &&
              proxyDefaults === undefined) {
